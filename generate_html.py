...
function stateCol(name){
  ...
  if(cnt===0)return 'rgba(100,116,139,0.45)';
  const t=Math.min(cnt/maxCntContext(),1);
  if(t<0.08)return 'rgba(59,130,246,0.6)';
  if(t<0.25){const tt=(t-.08)/.17;return `rgba(${Math.round(59+tt*130)},${Math.round(130-tt*80)},${Math.round(246-tt*200)},${(0.6+tt*0.15).toFixed(2)})`; }
  if(t<0.55){const tt=(t-.25)/.3;return `rgba(${Math.round(189+tt*50)},${Math.round(50-tt*30)},${Math.round(46-tt*20)},${(0.75+tt*0.1).toFixed(2)})`; }
  return `rgba(${Math.round(220+t*20)},20,20,0.9)`;
}

function getAlt(name){
  const cnt=cntForState(name),mx=maxCntContext();
  return cnt===0?0.002:0.006+(cnt/mx)*0.04;
}
...
function initGlobe(){
  ...
  globe=Globe({animateIn:false})(el)
    ...
    .onPolygonHover(d=>{
      globe.polygonCapColor(dd=>{
        if(!d)return stateCol(dd.properties.name);
        return dd===d?'rgba(255,200,0,0.85)':stateCol(dd.properties.name);
      });
    })
